...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        dupes = orm.AddressBatch.objects.raw(
            "SELECT * FROM map_addressbatch WHERE tag in "
            "(SELECT tag FROM map_addressbatch GROUP BY tag HAVING (COUNT(tag) > 1))")

        seen_tags = set()
        to_delete_ids = set()
        for batch in dupes:
            if batch.tag in seen_tags:
                to_delete_ids.add(batch.id)
            else:
                seen_tags.add(batch.tag)

        orm.AddressBatch.objects.filter(id__in=to_delete_ids).delete()
