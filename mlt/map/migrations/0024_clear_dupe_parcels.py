...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        dupes = orm.Parcel.objects.raw(
            "SELECT * FROM map_parcel WHERE pl in "
            "(SELECT pl FROM map_parcel GROUP BY pl HAVING (COUNT(pl) > 1))")

        seen_pls = set()
        to_delete_ids = set()
        for parcel in dupes:
            if parcel.pl in seen_pls:
                to_delete_ids.add(parcel.id)
            else:
                seen_pls.add(parcel.pl)

        orm.Parcel.objects.filter(id__in=to_delete_ids).delete()
