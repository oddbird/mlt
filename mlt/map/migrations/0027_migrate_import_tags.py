...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for batch_data in orm.Address.objects.values(
            "imported_by", "import_timestamp", "import_source").distinct():

            # no need for gigantic "base" or one-address "web-ui" batches
            if batch_data["import_source"] in ["web-ui", "base"]:
                continue

            batch = orm.AddressBatch.objects.create(
                user_id=batch_data["imported_by"],
                timestamp=batch_data["import_timestamp"],
                tag=batch_data["import_source"])

            for address in orm.Address.objects.filter(
                imported_by=batch_data["imported_by"],
                import_timestamp=batch_data["import_timestamp"],
                import_source=batch_data["import_source"]):
